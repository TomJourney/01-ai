...
步骤2：计算完成后，把用户问题向量与所有片段向量的向量相似度排序，得到与用户问题向量相似度最高的10个片段向量（10是参数，可自定义）。
...
2. 为什么不直接在召回阶段选择3个片段：
   1.  相比于直接使用召回，使用召回+重排能够选择出与用户最相关的3个片段，<font color=red>准确度会更高</font>；
   2. <font color=red>因为 重排使用的相似度函数与召回不同</font>；
...
1. 定义：把用户问题与3个相关片段内容及向量发给大模型用于生成问题答案的过程；
...
1. 用户提问前的准备部分：
   1. 步骤1：把相关资料做分片；
   2. 步骤2：把所有分片都扔给embedding嵌入模型；
   3. 步骤3：embedding模型给每个片段都产生一个向量；
   4. 步骤4：把片段内容及向量保存到向量数据库中；
...
1. 用户提问后的回答部分：
   1. 步骤1：用户问题送入embedding模型；
   2. 步骤2：embedding模型把用户问题转为一个向量；
   3. 步骤3：把用户问题及向量存入向量数据库；
   4. 步骤4：向量数据库通过相似度计算出与用户问题最相关的10个片段；
   5. 步骤5：把10个片段送入cross-encoder模型执行重排，选出3个与用户问题最相关的片段；
   6. 步骤6：把3个最相关片段内容及向量，连同用户问题文本及其向量一并发给大模型；
   7. 步骤7：大模型分析并生成答案；
